...
describe("banter", () => {
  ...
  it("lists every roster face for a most-likely, leaders first", () => {
    ...
    expect(pollFaces(roster, votes, "sandbag").map((row) => row.id)).toEqual([
      "e",
      "c",
      "a",
      "b",
    ...
    expect(pollFaces(roster, votes, "sandbag", [], 4).map((row) => row.id)).toEqual(["e", "c", "a", "b"]);
