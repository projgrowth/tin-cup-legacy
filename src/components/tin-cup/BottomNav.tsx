...
export function BottomNav() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 px-3 pb-[max(0.35rem,env(safe-area-inset-bottom))] md:inset-x-auto md:left-1/2 md:top-3 md:bottom-auto md:-translate-x-1/2 md:px-0 md:pb-0">
      <div className="glass mx-auto flex w-full max-w-md items-stretch justify-between gap-0 border border-border/80 px-0.5 py-0.5 md:w-auto md:max-w-none md:gap-1 md:px-1.5 md:py-1">
        {items.map(({ to, label, icon: Icon, exact }) => (
          <Link
            key={to}
            to={to}
            activeOptions={{ exact }}
            className="press t-micro relative flex min-h-11 flex-1 flex-col items-center justify-center gap-0.5 rounded-md px-1 py-1.5 font-medium tracking-[-0.01em] text-muted-foreground transition-colors after:absolute after:inset-x-3 after:top-0.5 after:h-px after:rounded-full after:bg-foreground after:opacity-0 data-[status=active]:text-foreground data-[status=active]:after:opacity-100 md:min-h-0 md:flex-row md:gap-2 md:px-3 md:py-1.5 md:after:hidden"
          >
