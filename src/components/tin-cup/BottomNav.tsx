...
export function BottomNav({ live = false }: { live?: boolean }) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 px-3 pb-[max(0.35rem,env(safe-area-inset-bottom))] lg:inset-x-auto lg:left-1/2 lg:bottom-3 lg:-translate-x-1/2 lg:px-0 lg:pb-0">
      <div className="mx-auto flex w-full max-w-md items-stretch justify-between gap-0.5 rounded-2xl border border-border bg-[var(--glass)] px-1 py-0.5 backdrop-blur-xl lg:w-auto lg:max-w-none lg:gap-1 lg:px-1.5 lg:py-1">
        {items.map(({ to, label, icon: Icon, exact }) => (
          <Link
            key={to}
            to={to}
            activeOptions={{ exact }}
            className="press t-micro relative flex min-h-11 flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-1.5 font-semibold tracking-[-0.01em] text-muted-foreground transition-colors data-[status=active]:bg-white/10 data-[status=active]:text-foreground lg:flex-row lg:gap-2 lg:px-3.5 lg:py-2"
          >
            ...
            <span className="text-[0.65rem] md:text-[0.8125rem]">{label}</span>
          </Link>
        ))}
      </div>
    </nav>
